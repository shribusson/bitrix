<template>
	<div>
		<div :id="'post' + postData.id" class="postinfo"
		     v-if="/*!longBlock && */postData.id">
			<div
				@drop='onDrop($event, postData, nodeEmp)'
				@dragenter="onDragEnter($event, postData, index, mode)"
				@dragleave="onDragLeave($event, postData)"
				@dragover.prevent
			>
				<!-- Хедер поста за который тянем пост -->
				<div class="title" :draggable="editMode" :data="idPost = postData.id"
				     @dragstart="startDrag($event, postData, nodeEmp, 'post')"
				     @dragend="dragEnd($event, postData, nodeEmp, 'post')">
					<a draggable="false" class="title_a" href="javascript:void(0);"
					   @mousedown="openPostMouseDown($event)"
					   @click="openPostClick($event, postData.id)">{{ postData.name }}</a>
					<button v-if="showCollapseBtn" class="ui-btn ui-btn-xs ui-btn-link" :class="{'ui-btn-icon-angle-up': !postCollapse, 'ui-btn-icon-angle-down': postCollapse}" @click="postCollapse = !postCollapse"></button>
				</div>
				<UserList :users="postData.users"
				          :is-manager="postData.isManager"
				          :hide-user-link="(nodeEmp.level < 4)"
				          :postId="postData.id"
				          v-if="showUsers"
				/>
				<!-- Окошко для перетаскивания юзера -->
				<div class="droppable-overlay-user" v-if="isDropActiveId === postData.id && mode === 'user'">
					<div class="droppable-overlay-title-user">{{ dropAreaText }}</div>
				</div>
			</div>
			...
			<div class="functions" v-if="showFunctions">
				<div class="title title_inline">
					Функции
					<button class="ui-btn ui-btn-xs ui-btn-link" :class="{'ui-btn-icon-angle-up': !functionsCollapse, 'ui-btn-icon-angle-down': functionsCollapse}" @click="functionsCollapse =!functionsCollapse"></button>
				</div>
				<div class="content" v-if="!functionsCollapse">
					{{ postData.functions }}
				</div>
			</div>
			<div class="functions" v-if="showCkp">
				<div class="title">
					ЦКП
				</div>
				<div class="content" v-html="postData.ckp"></div>
			</div>
		</div>
	</div>
</template>
...
export default {
	...
	computed: {
		...
		/*postData() {
		  let data = this.longBlock ? {} : {
			id: this.nodeEmp.ID_POST,
			name: this.nodeEmp.NAME_POST,
			isManager: parseInt(this.nodeEmp.SHIEF) > 0,
			functions: this.nodeEmp.FUNCTIONS,
			ckp: this.nodeEmp.CKP,
			users: this.nodeEmp.hasOwnProperty('EMPLOYEE') ? this.nodeEmp.EMPLOYEE : []
		  }
		  return data;
		},*/
		showCollapseBtn() {
			return parseInt(this.nodeEmp.level) >= 5;
		},
		showCkp() {
			return parseInt(this.nodeEmp.level) >= 6 && !this.postCollapse;
		},
		showFunctions() {
			return parseInt(this.nodeEmp.level) >= 6 && !this.postCollapse;
		},
		showUsers() {
			return parseInt(this.nodeEmp.level) < 5 || !this.postCollapse;
		},
		wideBlock() {
	...
	mounted() {
		this.$root.$on('collapse.posts', () => {this.postCollapse = true});
		this.$root.$on('collapse.functions', () => {this.functionsCollapse = true});
		this.$root.$on('expand.posts', () => {this.postCollapse = false});
		this.$root.$on('expand.functions', () => {this.functionsCollapse = false});
	},
	methods: {
		...
		findNode(id, currentNode, action) {
			let i,
				currentChild,
				result;
			if (id == currentNode.ID) {
				...
				if (currentNode.children) {
					for (i = 0; i < currentNode.children.length; i += 1) {
						currentChild = currentNode.children[i];
						result = this.findNode(id, currentChild, action);
						if (result !== false && !this.result_found && !this.searchBlock) {
							...
							if (this.searchBlock.level != this.lvl) {
								...
								this.takeChild(this.searchBlock)
							}
		...
		addDropElem(container, postId, elem, typeEvent) {
			if (this.mode == 'user') {
				...
				//     if(typeEvent == 'move') {
				//       this.find(Number(postId), container.ID, this.departmentData, 'del')
				//     } else if(typeEvent == 'copy') {
			...
			if (this.editMode) {
				if (this.mode == 'user') {
					if (event.getModifierState("Shift") || event.getModifierState("Control")) {
						this.$store.commit('setIsDropActiveId', postData.id);
						if (event.getModifierState("Shift")) {
							this.$store.commit('setIsDropActiveText', 'Переместить сотрудника');
						}
						if (event.getModifierState("Control")) {
							this.$store.commit('setIsDropActiveText', 'Скопировать сотрудника');
						}
					}
				} /*else if (this.mode == 'post') {
          if (event.getModifierState("Shift")) {
	...
	watch: {},
	components: {Icon, UserList, CardSection},
	props: ["postData", "nodeEmp", "treeNode", "hideCkp", "hasChildren"],
};
</script>
