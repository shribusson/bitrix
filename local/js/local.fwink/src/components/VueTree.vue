...
export default {
  ...
  updated() {
    this.$nextTick(function() {
      setTimeout(function() { // костыль..
        const nodes = this.nodeDataList.filter(function (node) {
          return node.data.level === 6;
        });
        let maxHeight = 0;
        let nodesList = [];
        nodes.forEach(function (element) {
          const htmlEl = document.querySelector('[data-key="' + element.data._key + '"]');
          nodesList.push(htmlEl);
        ...
        nodesList.forEach(function (element) {
          element.style.height = this.formatDimension(maxHeight);
          element.style.transform = 'translate(-50%, -' + this.formatDimension(this.config.nodeHeight / 2) + ')';
        }.bind(this));
      }.bind(this), 1000);
    }.bind(this));
  },
  methods: {
    init() {
      ...
      this.$refs.container.addEventListener('mousewheel', (event) => {
        if (event.ctrlKey) {
    ...
    getWidth(node) {
      ...
      if(node.data.level === 1 || node.data.level === 2) {
        ...
        if(node.hasOwnProperty('children') && node.children.length) {
          multiply = node.children.length >= 3 ? 2.5 : node.children.length;
        }
      }
      return this.formatDimension(this.config.nodeWidth*multiply);
    },
    getTranslate() {
    ...
    draw() {
      ...
      window.testVar = this; // todo: remove
      links
          .enter()
          .append('path')
          .style('opacity', 0)
