<template >
    <div :id="'block' + nodeEmp.ID" class="section" :class="{wide: wideBlock}" v-if="!hide" :style="customBlockStyles"
         @dragenter="onDragEnter($event, '', '', mode)"
         @dragleave="onDragLeave($event, '')"
    >
        <!-- Хедер блока за который тянем Блок -->
        <div class="control-block" v-if="editMode && canEdit" :draggable="editMode"
             @dragstart="startDrag($event,'', nodeEmp, 'block')"
             @dragend="dragEnd($event, '', nodeEmp, 'block')">
        ...
        <div class="header" v-if="!longBlock" :style="customHeaderStyles">{{nodeEmp.NUMBER}} {{ nodeEmp.NAME }}</div>

        <!-- Окошко для перетаскивания поста -->
        <div v-if="!longBlock && nodeEmp.ID" class="droppablePost"
             :class="{ready: (isDropActiveId === nodeEmp.ID && mode === 'post')}"
          @drop='onDrop($event, "", nodeEmp)'
          @dragover.prevent
        >
          <div class="droppable-overlay" v-if="!longBlock && isDropActiveId === nodeEmp.ID && mode === 'post'">
            <div class="droppable-overlay-title">{{dropAreaText}}</div>
          </div>
        </div>
        <CardPost :nodeEmp="nodeEmp" :post-data="postData" v-for="postData in nodeEmp.POSTS" :index="postData.id"/>
        <div class="long-container" v-if="longBlock">
          <div class="childs">
            <CardSection v-for="child in nodeEmp.childs"
                        :block="child"
                        @openpost="openPost"
...
export default {
  ...
  computed: {
    ...
    longBlock() {
      return parseInt(this.nodeEmp.level) === 6 && this.nodeEmp.hasOwnProperty('childs');
    },
    childSection() {
      return parseInt(this.nodeEmp.level) >= 6 && !this.nodeEmp.hasOwnProperty('childs');
    }
  ...
  watch: {},
  components: { Icon, UserList, CardSection, CardPost },
