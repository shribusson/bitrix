 ...
 * В наследниках этого класса могут быть опеределены функции для получения дополнительной информации в значение поля.
 ...
 * Class Base
 *
 * @package Local\Fwink\Fields\Value
...
class Base implements ValueFromDbInterface
{
    ...
    /**
     * Функция по select'у получает значение поля из результата запроса к базе данных.
     *
    ...
    private function extractFieldValueFromRawRow(array $row, $selectFields)
    {
        if (is_array($selectFields)) {
            $rawValue = array_filter($row, static function ($key) use ($selectFields) {
    ...
    protected function getMainValue($rawValue)
    {
        ...
        if (is_array($this->fieldNameForValue)) {
            $mainValue = array_filter($rawValue, function ($key) {
     ...
     * @return mixed
     */
    protected function getFieldFromRawValue($fieldName)
    {
