<script>console.log("⚠️ Подключен шаблон template.php");</script>

<?php
echo "<div style='color:red; font-weight:bold;'>[DEBUG: Шаблон tasks.list.items активен]</div>";
...
//	$createUrl = $createUrl.(!str_contains($createUrl, "?") ? "?" : "&")."PARENT_ID=".$task["ID"];
	...
	foreach ($arResult['COLUMNS_IDS'] as $columnId)
	{
        ...
        if (!empty($task['UF_PRIORITY'])) {
            $rsEnum = CUserFieldEnum::GetList([], ['ID' => $task['UF_PRIORITY']]);
            if ($arEnum = $rsEnum->GetNext()) {
                echo '<b><small>⇒ Приоритет:</b> ' . htmlspecialcharsbx($arEnum['VALUE']) . '</small>';
            }
        }
    break;
    ...
    case 'UF_CATEGORY':
        if (!empty($task['UF_CATEGORY'])) {
            $rsEnum = CUserFieldEnum::GetList([], ['ID' => $task['UF_CATEGORY']]);
            if ($arEnum = $rsEnum->GetNext()) {
                echo '<b><small>⇒ Категория:</b> ' . htmlspecialcharsbx($arEnum['VALUE']) . '</small>';
            }
        }
    break;
