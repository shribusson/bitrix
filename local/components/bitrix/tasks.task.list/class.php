...
{
	protected static function decodeUserFields(array &$tasks): void
{
    $enumIds = [];

    foreach ($tasks as $task)
    {
        if (!empty($task['UF_PRIORITY'])) {
            $enumIds[] = $task['UF_PRIORITY'];
        }
        if (!empty($task['UF_CATEGORY'])) {
            $enumIds[] = $task['UF_CATEGORY'];
        }
    }
    ...
    $res = $enum->GetList([], ['ID' => array_unique($enumIds)]);
    $map = [];
    ...
    foreach ($tasks as &$task)
    {
        if (!empty($task['UF_PRIORITY']) && isset($map[$task['UF_PRIORITY']])) {
            $task['UF_PRIORITY'] = $map[$task['UF_PRIORITY']];
        }

        if (!empty($task['UF_CATEGORY']) && isset($map[$task['UF_CATEGORY']])) {
            $task['UF_CATEGORY'] = $map[$task['UF_CATEGORY']];
        }
    }
	}
	protected static function getEnumMap(string $fieldName): array
{
    $enum = new \CUserFieldEnum();
    $result = $enum->GetList([], ['USER_FIELD_NAME' => $fieldName]);
...
$priorityMap = self::getEnumMap('UF_PRIORITY');
$categoryMap = self::getEnumMap('UF_CATEGORY');
	...
	public static function getGridRows(array $taskIds, array $arParams): array
	{
		...
		self::decodeUserFields($tasks); //Может заработает хз
	...
	protected function display()
	{
		...
		if ($this->errors->checkNoFatals())
		{
			...
			{
				// Получаем словари для ENUM-полей
        $priorityMap = $this->getEnumMap('UF_PRIORITY');
        $categoryMap = $this->getEnumMap('UF_CATEGORY');
