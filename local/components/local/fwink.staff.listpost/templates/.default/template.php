        ...
        <div class="ui-toolbar-right-buttons">
            ...
            if($arResult['ACCESS']['create']){ ?>
                <button class="ui-btn ui-btn-primary ui-btn-icon-add"
                        onclick="BX.iTrack.Chart.PostList.newPost()">
                    <span class="ui-btn-text">Добавить должность</span>
                </button>
            <?php } ?>
        </div>
    </div>
...
<script>
    BX.ready(function(){
        BX.iTrack.Chart.PostList.init({
            gridId: '<?=$gridID?>',
            serviceUrl: '<?=$arResult['SERVICE_URL']?>',
            sign: '<?=$arResult['SIGN']?>',
            signedParamsEdit: '<?=$arResult['SIGNED_PARAMS_EDIT']?>',
            editUrl: '<?=$arResult['EDIT_URL']?>'
        });
    });
</script>
