...
$enc = SimpleEncDecriptStringToNumber::encrypt($whichdomainsendedrequest) . $randstring;
...
foreach ($arResult['COLUMNS'] as $key => $val) {
	if ($arResult['COLUMNS'][$key]['field'] == 'NAME_POST') {
		$arResult['COLUMNS'][$key]['width'] = '200';
	}
	if ($arResult['COLUMNS'][$key]['field'] == 'ID_SHIEF_POST_USERB24') {
		$arResult['COLUMNS'][$key]['width'] = '200';
	}
	...
	if ($arResult['COLUMNS'][$key]['field'] == 'ID_PORTAL') {
		$arResult['COLUMNS'][$key]['width'] = '30';
		$arResult['COLUMNS'][$key]['title'] = '';
	}
}
...
<div>
    <table class="bx-layout-inner-inner-table top-menu-mode grid-mode">
                    ...
                    <div id="uiToolbarContainer" class="ui-toolbar">
                        ...
                        <div class="ui-toolbar-right-buttons">
                            ...
                            <button class="ui-btn ui-btn-primary ui-btn-icon-add"
                                ...
                                <span class="ui-btn-text" idn="116">Добавить</span>
                            </button>
                        </div>
        ...
        <tr>
            <td>
                <div id="example-table" idn="10776"></div>
    ...
    (function () {
        ...
        BX.HelpsDesk.Listext = {
            ...
            openPageAddUser: function () {
                var url = document.location.pathname + '?add=new&mode=pages&page=post&access=whateverdomaintokenfordemo';
                //var url='?add=new&mode=pages&page=post&access=whateverdomaintokenfordemo';
            ...
            actionDelete: function (element) {
                var idCell = element.getRow().getData().ID;
                var url = document.location.pathname;
                //+ '?add=new&mode=pages&page=post&access=whateverdomaintokenfordemo';
                ...
                BX.PopupMenu.show('demo-popup-menu', BX(idelementCell), [
                    {
                        ...
                        href: '#', // Ссылка
                        className: 'menu-popup-item menu-popup-no-icon idn23', // Дополнительные классы
                        /*className: 'menu-popup-item menu-popup-item-accept',*/
                        onclick: function (e, item) {
                            //BX.PreventDefault(e);
                            // Событие при клике на пункт
                            // console.log('..click:' + idCell) /helpdsk/staff/959/
                            console.log('?element_id=' + idCell + '&mode=404');
                            var url = document.location.pathname + '' + cell + '/';
                            BX.SidePanel.Instance.open('?element_id=' + idCell + '&mode=404?link_utm=<?=$enc;?>', {
                                cacheable: false
                            });
                    ...
                    },
                    {
                        ...
                        href: '#', // Ссылка
                        className: 'menu-popup-item menu-popup-no-icon idn39d', // Дополнительные классы
                        /*className: 'menu-popup-item menu-popup-item-accept',*/
                        onclick: function (e, item) {
                            //BX.PreventDefault(e);
                            var url = document.location.pathname + '' + cell + '/';
                            /*BX.SidePanel.Instance.open('?element_id='+idCell+'&mode=pages&page=post&access=whateverdomaintokenfordemo&link_utm=<?=$enc;?>', {
                                cacheable: false
                            });*/
                            console.log('..send dictionary 471');
                            // window.BX24.openApplication(url);
                            window.BX24.openApplication({
                                mode: 'pages',
                                page: 'post',
                    ...
                    },
                    {
                        ...
                        href: '#', // Ссылка
                        className: 'menu-popup-item menu-popup-no-icon idneb', // Дополнительные классы
                        /*className: 'menu-popup-item menu-popup-item-accept',*/
                        onclick: function (e, item) {
                            //BX.PreventDefault(e);
                            // Событие при клике на пункт
                            ...
                            BX.HelpsDesk.Listext.actionDelete(cell);
